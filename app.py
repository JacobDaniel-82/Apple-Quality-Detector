...
def load_classifier_model():
    import tensorflow as tf
    from keras.layers import InputLayer # Note: imported from keras, not tensorflow.keras
...
def predict_apple_disease(model, img_array):
    try:
        ...
        # Based on your model summary: 1 output node (Dense 1)
        # In binary classification, the value is between 0 and 1
        score = float(predictions[0][0])
        
        # Standard Binary Logic:
        # Usually 0 = Healthy, 1 = Rotten
        if score < 0.5:
            category = "Apple is Healthy"
            confidence = (1 - score) * 100
        else:
            category = "Apple is Rot"
